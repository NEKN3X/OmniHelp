...
test('synonym', () => {
  ...
  expect(synonym('(xyz|123|あいう)abc')).toEqual([[['xyz', '123', 'あいう'], 'abc']]);
});
test('literal', () => {
  ...
  expect(literal('abc(xyz|123|あいう)')).toEqual([[['abcxyz', 'abc123', 'abcあいう'], '']]);
  expect(literal('abc(xyz|123)def')).toEqual([[['abcxyzdef', 'abc123def'], '']]);
});
...
test('expand', () => {
  ...
  expect(expand('abc(xyz|123|あいう)')).toEqual(['abcxyz', 'abc123', 'abcあいう']);
  expect(expand('abc(xyz|123)def')).toEqual(['abcxyzdef', 'abc123def']);
  expect(expand('abc(xyz|123)def(abc|xyz)')).toEqual(['abcxyzdefabc', 'abcxyzdefxyz', 'abc123defabc', 'abc123defxyz']);
